...
export const StudentForm = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === "personal") {
      const cleanPhone = String(formData.phone || "").trim().replace(/\D/g, "");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/75 backdrop-blur-md animate-in fade-in duration-200 modal-backdrop-container">
      <div className="skeuo-card w-full sm:max-w-lg shadow-2xl animate-in slide-in-from-bottom sm:zoom-in-95 duration-200 max-h-[92vh] sm:max-h-[90vh] flex flex-col overflow-hidden rounded-t-3xl sm:rounded-3xl border-t sm:border border-slate-300 dark:border-slate-800">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto custom-scrollbar flex-1 pb-8 modal-scrollable-content">
          {/* Photo Section */}
            ...
            <>
              <div>
                ...
                <input
                  ...
                  className={clsx(
                    "skeuo-input w-full px-4 py-2.5 text-xs font-medium tracking-wider",
                    formData.phone.length > 0 &&
                      formData.phone.length < 10 &&
